...
};
{
  ...
  function drawDetail(detail, angle) {
    ...
    var rt = +leftAngleInput.value || detail.rt.radius || 0;
    var rb = detail.rb.radius || 0;
    var lb = detail.lb.radius || 0;
    var lt = detail.lt.radius || 0;
  ...
  leftAngleInput.addEventListener('input', function (event) {
    var radius = +event.target.value;
    if (isNaN(radius) || radius < 0 || radius > 200) return;

    var newDetail = _objectSpread(_objectSpread({}, detail), {}, {
      rt: {
        radius: radius
      }
    });
